...
export const HeroSection = () => {
  ...
  <section className="relative overflow-hidden border-b border-border/70">
    ...
    <div className="container relative mx-auto px-4 py-6 md:py-8">
      <div className="mb-8 flex items-center justify-between border-y border-border/70 py-3 font-mono text-[10px] uppercase tracking-[.18em] text-muted-foreground">
      ...
      <div className="grid items-stretch gap-7 lg:grid-cols-[1.16fr_.84fr]">
        ...
        <aside ref={portraitRef} className="portrait-stage journal-card group mx-auto aspect-[4/5] w-full max-w-[440px] animate-slide-up lg:self-center" style={{ animationDelay: ".12s" }}>
          ...
          <div className="absolute inset-x-0 top-0 flex items-center justify-between p-5 font-mono text-[9px] uppercase tracking-[.18em] text-foreground drop-shadow-md">
          ...
          <div className="absolute inset-x-0 bottom-0 p-5 md:p-7">
            <div className="mb-5 flex flex-wrap gap-2">
              {focusAreas.map((area) => (
                <span key={area} className="rounded-full border border-white/15 bg-background/55 px-3 py-1.5 font-mono text-[9px] uppercase tracking-wider text-foreground backdrop-blur-md">
                  {area}
                </span>
              ))}
            </div>
            <div className="border-t border-white/20 pt-5">
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-3 font-mono text-[10px] uppercase tracking-wider text-foreground/75">
                <span>{aboutData.title}</span>
                <span className="flex items-center gap-1.5"><MapPin className="h-3 w-3 text-primary" /> Vietnam</span>
              </div>
              <a href={aboutData.social.github} target="_blank" rel="noreferrer" className="mt-5 flex items-center justify-between rounded-lg border border-white/20 bg-background/55 px-4 py-3 font-mono text-[10px] uppercase tracking-wider backdrop-blur-md transition-colors hover:border-primary hover:text-primary">
      ...
      <div className="hero-marquee mt-7 overflow-hidden border-y border-border/70 py-3 font-mono text-[9px] uppercase tracking-[.2em] text-muted-foreground">
